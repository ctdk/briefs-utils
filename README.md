...
Creates new BrieFS volumes. It formats new filesystems with the packed directory trie layout used by BrieFS 0.8.0+.
...
The idea with `fsck.briefs` is that it will repair broken, mangled, and mutilated BrieFS volumes. It now performs a growing list of consistency checks, including CRC32C verification of journal records and extent chain blocks, validates the packed directory trie pages used by BrieFS 0.8.0+, and can repair many common problems. Repairs can be run selectively with `--repair-only=allocator,extents,trie,links` or limited to safe compaction with `--optimize`.
...
GLOBAL OPTIONS:
   ...
   --repair             attempt to repair found errors (default: false)
...
A FUSE bridge for BrieFS so you can mount BrieFS volumes without the commitment of loading and/or battling with a kernel module. Currently read-only and basic. Supports the packed directory trie format introduced in BrieFS 0.8.0.
