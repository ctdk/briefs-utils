...
// prefix size below.
//
// Each struct below carries a //go:briefs-disk marker; cmd/gendisk
// generates the byte-exact little-endian MarshalBinary/UnmarshalBinary in
// gen_disk.go (regenerated by `go generate`), and each Marshal() is a thin
// adapter over it.  Field sizes and offsets are pinned by the generated
// size assertions, guaranteeing byte-for-byte compatibility with the
// kernel's on-disk format.
...
// Fixed record payload sizes (bytes). Only the ones with live consumers are
// named; every record's size is additionally pinned by the gendisk size
// assertions in gen_disk.go.
const (
